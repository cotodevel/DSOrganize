...
{
	static char tTag[1024];
	static char andTag[1024];
...
uint16 getColorIndex(uint16 color, HTML_RENDERED *htmlPage)
{
	...
	for(u32 x=0;x<htmlPage->maxColors;x++)
	{
		if(htmlPage->colors[C_START+x] == color)
			return C_START+x;
	}
	
	if(htmlPage->maxColors == 512) // ran out of stack
		return C_START;
						...
						{					
							...
							{	
								...
								if(strcmp(tmpTag, "submit") == 0 || strcmp(tmpTag, "reset") == 0 || strcmp(tmpTag, "button") == 0)
								{
									...
									htmlPage->formCodes[htmlPage->maxFormControls-1].width = getStringWidth(tmpTag, (uint16 **)font_gautami_10) + 10;									
									htmlPage->formCodes[htmlPage->maxFormControls-1].height = 15;
