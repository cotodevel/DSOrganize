...
WIFI_PROFILE *dsoProfiles = NULL;
char proxy[256];
char dldiFile[256];
char homePage[256];
char downloadLocation[256];
int mode = HOME;
int lastMode;
bool swapAB = false;
bool textFixedWidth = false;
bool ircFixedWidth = false;
bool secondClickOpens = true;
bool autoHide = true;
bool imageLandscape = true;
bool keyClick = false;
...
uint16 **setTextEditorFont()
{
	if(textFixedWidth)
	{
		return (uint16 **)font_profont_9;
	}
	else
	{
		return (uint16 **)font_arial_9;
	}
}
...
void loadSettings()
{
	...
	if(getSetting("General", "NormalLoader", sStr))
	{
		...
		if(strcmp(sStr, "internal") == 0)
			normalBoot = BOOT_INTERNAL;
	...
	if(getSetting("Startup", "DefaultScreen", sStr))
	{
		...
		if(strcmp(sStr,"calendar") == 0)
			gotoApplication(0,false); //CALENDAR
		if(strcmp(sStr,"dayview") == 0)
			gotoApplication(1,false); //DAYVIEW
		if(strcmp(sStr,"address") == 0)
			gotoApplication(2,false); //ADDRESS;
		if(strcmp(sStr,"todo") == 0)
			gotoApplication(3,false); //TODOLIST;
		if(strcmp(sStr,"scribble") == 0)
			gotoApplication(4,false); //SCRIBBLEPAD;
		if(strcmp(sStr,"browser") == 0)
			gotoApplication(5,false); //BROWSER;
		if(strcmp(sStr,"calculator") == 0)
			gotoApplication(6,false); //CALCULATOR;
		if(strcmp(sStr,"irc") == 0)
			gotoApplication(7,false); //IRC;
		if(strcmp(sStr,"webbrowser") == 0)
			gotoApplication(8,false); //WEBBROWSER;
	}
