...
#define READ(_gif,_buf,_len)                                     \
  (((GifFilePrivateType*)_gif->Private)->Read ?                   \
    ...
    DRAGON_fread(_buf,1,_len,((GifFilePrivateType*)_gif->Private)->File))
 ...
 *****************************************************************************/
GifFileType *
DGifOpenFileHandle(DRAGON_FILE *FileHandle) {
...
DGifGetScreenDesc(GifFileType * GifFile) {
    ...
    if (Buf[0] & 0x80) {    /* Do we have global color map? */
        ...
        for (i = 0; i < GifFile->SColorMap->ColorCount; i++) {
			...
			*/
			GifFile->SColorMap->Colors[i].color = RGB15(Buf[0] >> 3,Buf[1] >> 3,Buf[2] >> 3);			
        }
    } else {
        GifFile->SColorMap = NULL;
    }
    ...
    if (Buf[0] & 0x80) {    /* Does this image have local color map? */
        ...
        for (i = 0; i < GifFile->Image.ColorMap->ColorCount; i++) {
			...
			*/
			...
			GifFile->Image.ColorMap->Colors[i].color = RGB15(Buf[0] >> 3,Buf[1] >> 3,Buf[2] >> 3);
        }
    ...
    do {
        ...
        switch (RecordType) {
          case IMAGE_DESC_RECORD_TYPE:
              ...
              sp->RasterBits = (char *)malloc(ImageSize * sizeof(GifPixelType));
