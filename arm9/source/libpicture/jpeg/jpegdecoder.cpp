  ...
  int block_len_x, int block_len_y)
{
  ...
  // modified to use files instead
  /*uchar *tData =*/ cb->Pdata = (uchar *)alloc(cb->block_size * block_num_x * block_num_y);
  ...
  // for caching later
  cb->Pdata = (uchar *)alloc(cb->block_size);*/
  ...
  BLOCK_TYPE *buffer)
{
  ...
  // modified to use files instead
  //DRAGON_fseek(cb->Pfile, block_x * cb->block_size + block_y * (cb->block_size * cb->block_num_x), SEEK_SET);
  //DRAGON_fread(buffer, 1, cb->block_size, cb->Pfile);
  ...
  BLOCK_TYPE *buffer)
{
  ...
  // modified to use files instead
  //DRAGON_fseek(cb->Pfile, block_x * cb->block_size + block_y * (cb->block_size * cb->block_num_x), SEEK_SET);
  //DRAGON_fwrite(buffer, 1, cb->block_size, cb->Pfile);
  ...
  int block_x, int block_y)
{
  ...
  // modified to user files instead
  //DRAGON_fseek(cb->Pfile, block_x * cb->block_size + block_y * (cb->block_size * cb->block_num_x), SEEK_SET);
  //DRAGON_fread(cb->Pdata, 1, cb->block_size, cb->Pfile);

  return (BLOCK_TYPE *)(cb->Pdata + block_x * cb->block_size + block_y * (cb->block_size * cb->block_num_x));
  //return (BLOCK_TYPE *)(cb->Pdata);
}
//------------------------------------------------------------------------------
...
int jpeg_decoder::decodeFake(void)
{
