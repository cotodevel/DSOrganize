...
//#define SUPPORT_MMX
//------------------------------------------------------------------------------
#define JPGD_INBUFSIZE       4096*64*2
//------------------------------------------------------------------------------
// May need to be adjusted if support for other colorspaces/sampling factors is added
#define JPGD_MAXBLOCKSPERMCU 10
//------------------------------------------------------------------------------
#define JPGD_MAXHUFFTABLES   8*2
#define JPGD_MAXQUANTTABLES  4*2
#define JPGD_MAXCOMPONENTS   4*2
#define JPGD_MAXCOMPSINSCAN  4*2
//------------------------------------------------------------------------------
// Increase this if you increase the max width!
#define JPGD_MAXBLOCKSPERROW 6144*2
//------------------------------------------------------------------------------
// Max. allocated blocks
#define JPGD_MAXBLOCKS    200
//------------------------------------------------------------------------------
...
{
  uchar *Pdata; // left in for caching
  //DRAGON_FILE *Pfile;
...
public:
  ...
  virtual int read(uchar *Pbuf, int max_bytes_to_read, bool *Peof_flag)
  {
    ...
    int bytes_read = DRAGON_fread(Pbuf, 1, max_bytes_to_read, Pfile);

    if (bytes_read < max_bytes_to_read)
    {
      /*if (DRAGON_ferror(Pfile))
      {
        error_flag = true;
        return (-1);
      }*/
  ...
  int get_size(void)
  {
    ...
    int loc = DRAGON_ftell(Pfile);

    DRAGON_fseek(Pfile, 0, SEEK_END);

    int size = DRAGON_ftell(Pfile);

    DRAGON_fseek(Pfile, loc, SEEK_SET);
