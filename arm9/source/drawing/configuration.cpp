...
bool isAPScreen()
{
	return ((wifiState == WIFI_EDITSETTING) && (wifiCursor == -2));
}
...
void drawTopConfiguration()
{
	...
	else
	{
		...
		drawKeyboardSpecific(3, BUTTON_X, l_swap, fb_backBuffer());
		...
		if(keysPressed() & KEY_TOUCH)
		{
			...
			if((t.touchXpx > 13) && (t.touchYpx > 19) && (t.touchXpx < 241) && (t.touchYpx < 33))
			{
				...
				if(tCur == -2)
					moveCursorAbsolute(strlen(str));
				else if(tCur == -1)
					resetCursor();
				else
					moveCursorAbsolute(tCur);
		...
		fb_dispString(centerOnPt(getLLocation(), str1, (uint16 **)font_arial_9),165,str1);	
		fb_dispString(centerOnPt(getRLocation(), str2, (uint16 **)font_arial_9),165,str2);	
		...
		fb_dispString(253-getStringWidth(str, (uint16 **)font_gautami_10),3, str);
	}
}
...
void configListCallback(int pos, int x, int y)
{
	...
	abbreviateString(str, (list_right - 27) - (list_left + 15), (uint16 **)font_arial_9);		
...
{	
	...
	else if(configCursor >= PAGE11) // page 11
	{
		...
		else if(wifiState == WIFI_EDITSETTING)
		{
			if(wifiCursor == -2)
			{
				...
				drawListBox(list_left, LIST_TOP, list_right, LIST_BOTTOM, getCursor(), numap, l_nowifi, configListCallback);
				...
				drawScrollBar(getCursor(), numap, default_scroll_left, DEFAULT_SCROLL_TOP, DEFAULT_SCROLL_WIDTH, DEFAULT_SCROLL_HEIGHT);
			}
			else
			{
				drawKeyboardSpecific(3, BUTTON_X, l_swap, bg_backBuffer());
				...
				if(keysPressed() & KEY_TOUCH)
				{
					...
					if((t.touchXpx > 13) && (t.touchYpx > 19) && (t.touchXpx < 241) && (t.touchYpx < 33))
					{
						...
						if(tCur == -2)
							moveCursorAbsolute(strlen(wifiInput));
						else if(tCur == -1)
							resetCursor();
						else
							moveCursorAbsolute(tCur);
	...
	if(configCursor >= PAGE10)
	{
		...
		{
			sprintf(str,"%c\a\a%s", BUTTON_START, l_edit);	
			bg_dispString(centerOnPt(128,str, (uint16 **)font_gautami_10), 3, str);
		}
		else if(wifiState == WIFI_NORMAL)
		{
	...
	{	
		...
		{
			drawButtonTexts(l_done, l_cancel);
		}
		else
		{
...
void leftAction()
{
	...
	{
		if(saveFormat == SAVE_PNG)
			saveFormat = SAVE_BMP;
		else if(saveFormat == SAVE_BMP)
			saveFormat = SAVE_PNG;
	}
...
void rightAction()
{
	if(configCursor >= PAGE10 && wifiState != WIFI_NORMAL)
	{
		...
		{			
			if(getCursor() < (int)strlen(wifiInput))
	...
	{
		if(getCursor() < (int)strlen(defaultSavePath))
			moveCursorRelative(CURSOR_FORWARD);
	}
	else if(configCursor == PROXY)
	{
		if(getCursor() < (int)strlen(getProxy()))
			moveCursorRelative(CURSOR_FORWARD);	
	}
	else if(configCursor == AUTOCONNECT)
		autoConnect = !autoConnect;
	...
	{
		if(getCursor() < (int)strlen(nickName))
			moveCursorRelative(CURSOR_FORWARD);
	}	
	else if(configCursor == ALTNICK)
	{
		if(getCursor() < (int)strlen(altnickName))
			moveCursorRelative(CURSOR_FORWARD);
	}
	else if(configCursor == IRCSERVER)
	{
		if(getCursor() < (int)strlen(ircServer))
			moveCursorRelative(CURSOR_FORWARD);	
	}
	else if(configCursor == SWAPAB)
		toggleABSwap();
	...
	{
		if(getCursor() < (int)strlen(getDownloadDir()))
			moveCursorRelative(CURSOR_FORWARD);
	}
}
...
void editConfigAction(char c)
{
	...
	int *pts = NULL;
	int numPts = getWrapPoints(3, 5, tmpBuffer, 13, 19, 239, 33, &pts, (uint16 **)font_gautami_10);
	free(pts);
