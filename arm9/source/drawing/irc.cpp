...
char getCurrentMode(SCREEN_BUFFER *tmp, char *nick)
{
	...
	{
		if(areEqual(tmp->nicks[i].nick, nick))
		{
...
bool isNickInScreen(SCREEN_BUFFER *tmp, char *toAdd)
{
	...
	for(u32 i=0;i<tmp->nickCount;i++)
	{
		if(areEqual(tmp->nicks[i].nick, toAdd))
			return true;
	}
...
void changeNick(SCREEN_BUFFER *tmp, char *toFind, char *toAdd)
{
	...
	for(u32 i=0;i<tmp->nickCount;i++)
	{
		if(areEqual(tmp->nicks[i].nick, toFind))
		{
			bool loopIt = true;
...
void drawTabs()
{
	...
	for(int i=0;i<tabCount;i++)
		widths[i] = getStringWidth(tabs[i].name, (uint16 **)font_gautami_10) + 10;	
...
void drawColors()
{
	...
	bold[4] = keyboardTextColor;
	underline[4] = keyboardTextColor;
	color[5] = keyboardTextColor;
	backspace[5] = keyboardTextColor;
...
void drawScrollBarIRC(int pos, int max)
{
	if(getHand() == HAND_RIGHT)
	{
		drawScrollBar(pos, max, default_scroll_left, DEFAULT_SCROLL_TOP + 32 - 9, DEFAULT_SCROLL_WIDTH, DEFAULT_SCROLL_HEIGHT - 32);
	}
	else
	{
		drawScrollBar(pos, max, LIST_LEFT_R + 18, DEFAULT_SCROLL_TOP + 32 - 9, DEFAULT_SCROLL_WIDTH, DEFAULT_SCROLL_HEIGHT - 32);
	}
}
...
void drawNickList()
{
	...
	drawListBox(list_left + 18, LIST_TOP + 23, list_right, LIST_BOTTOM - 9, listCursor, getIRCNickCount(), NULL, IRCListCallback);
	drawScrollBarIRC(listCursor, getIRCNickCount());
}
...
void parseMessage(char *msg)
{
	...
	if(strcmp(command, "JOIN") == 0)
	{
		...
		if(strlen(fromNick) == 0 || areEqual(fromNick, displayNick))
		{			
			// itsa me!
	...
	if(strcmp(command, "KICK") == 0)
	{
		...
		if(areEqual(toNick, displayNick)) // you've been kicked
		{
			sprintf(cStr, "\n%s", irc_youkicked);
	...
	if(strcmp(command, "NICK") == 0)
	{
		...
		if(areEqual(fromNick, displayNick))
		{
		...
		{		
			for(int i=1;i<tabCount;i++)
			{
				...
				{
					if(areEqual(tabs[i].name, fromNick)) // change it
	...
	switch(ircMode)
	{
		...
		case IRC_READY:
			...
			if(connected)
			{
				...
				char *tcpTemp = (char *)safeMalloc(IRC_RECEIVESIZE); // checked
				setGlobalError(ERROR_NONE);
...
int calcString(char *out)
{
	if(!strUpdated)
	{
		...
		return getCursor() - textBlock;		
	}
	
	strUpdated = false;
	
	if(getStringWidth(pointedText, (uint16 **)font_gautami_10) < T_THRESHOLD)
	{
		textBlock = 0;
	...
	if(getCursor() < textBlock) // we've moved back
	{
		textBlock = getCursor();
	...
	while(width < T_THRESHOLD)
	{
		if(pointedText[i] == 0)
		{
			...
			return getCursor() - textBlock; // we can return because if the cursor is not less and it took up less on the screen, it has to be on
		}
	...
	if(getCursor() <= i) // cursor is on the screen!
	{
		memcpy(out, pointedText + textBlock, i - textBlock);
		out[i - textBlock] = 0;
	...
	switch(getActiveSideTab())
	{
		case IRC_TAB_KEYBOARD:
		{
			...
			switch(ircMode)
			{
				...
				case IRC_CLOSEDCONNECTION:
				case IRC_CLOSED:
					drawButtonTexts(l_reconnect, l_swap);
			...
			bg_setDefaultClipping();
			setColor(genericTextColor);	
			sprintf(tBStr,"%c\a\a%s", BUTTON_START, tKStr);				
			bg_dispString(centerOnPt((SELECT_RIGHT + CLEAR_LEFT) / 2, tBStr, (uint16 **)font_gautami_10), 143, tBStr);
		...
		case IRC_TAB_NICKS:
		{
			switch(ircMode)
			{
				...
				case IRC_CONNECTED:
				case IRC_READY:	
					drawButtonTexts(l_query, l_append);
...
void ircSwapButton()
{
	switch(getActiveSideTab())
	{
		...
		case IRC_TAB_NICKS:
		{
			...
			for(int i=0;i<(int)strlen(tabs[activeTab].screen->nicks[listCursor].nick);i++)
				editIRCAction(tabs[activeTab].screen->nicks[listCursor].nick[i]);
...
void ircDown()
{
	switch(getActiveSideTab())
	{
		...
		case IRC_TAB_NICKS:
		{
			if(listCursor < getIRCNickCount() - 1)
				listCursor++;
			break;
		}
	}
...
bool isNicks()
{
	return getActiveSideTab() == IRC_TAB_NICKS;
}

bool isIRCKeyboard()
{
	return getActiveSideTab() == IRC_TAB_KEYBOARD;
}
