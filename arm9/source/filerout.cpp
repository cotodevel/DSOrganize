...
void sortList(int pos)
{
	if(pos > 1) // lets sort this fucker
		qsort(fileList, pos, sizeof(VCARD_FILE), compareVCard);
}
	...
	while(fType != FT_NONE)
	{
		if(fType == FT_FILE)
		{
			if(isVCard(tmpFile))
			{
				if(pos == 0)
				{
					fileList = (VCARD_FILE *)trackMalloc(sizeof(VCARD_FILE), "vCard file");
					pos = 1;
				}
				else
				{
					pos++;
					fileList = (VCARD_FILE *)trackRealloc(fileList, sizeof(VCARD_FILE) * pos);
				}
...
int addVCard()
{
	...
	fileList = (VCARD_FILE *)trackRealloc(fileList, (sizeof(VCARD_FILE)) * (tmpEntries + 1));
...
uint16 readNDSIcon(DRAGON_FILE *fFile, u32 offset, uint16 *data)
{
	char c;
	int x,y,z;
	uint16 pallate[16];
		...
		//{
			switch(DRAGON_FileExists(sc.path))
			{
				...
				case FT_FILE:{
					...
					{
						uint16 *tIcons[] = { 0, ndsgenericlarge, txtfilelarge, vcfilelarge, dpfilelarge, reminderfilelarge, picfilelarge, picfilelarge, todofilelarge, soundfilelarge, hbfilelarge, hbfilelarge, cantlaunchlarge, playlistfilelarge, 0, cantlaunchlarge, cantlaunchlarge };
