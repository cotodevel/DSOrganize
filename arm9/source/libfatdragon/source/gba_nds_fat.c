...
// push variables temp space
u32 fatLastCluster1;
u32 curWorkDirCluster1;
u32 wrkDirCluster1;
int wrkDirSector1;
int wrkDirOffset1;
...
u8 getCheckSum(DIR_ENT *entry)
{
	...
	for (aliasPos=0; aliasPos < 11; aliasPos++)
	{
		...
		chkSum = ((chkSum & 1) ? 0x80 : 0) + (chkSum >> 1) + (aliasPos < 8 ? entry->name[aliasPos] : entry->ext[aliasPos - 8]);
	}
...
u32 FAT_LinkFreeCluster(u32 cluster)
{
	...
	// Added by DragonMinded, zero out the cluster to fix corruption
	//--------------------------------------------------------------
	u32 tSect = FAT_ClustToSect(firstFree);
	u32 i;
	char tBuffer[BYTE_PER_READ];
	
	memset(tBuffer, FILE_FREE, BYTE_PER_READ);
	
	for(i=0;i<filesysSecPerClus;++i)
		disc_WriteSector(tSect+i,tBuffer);
	//--------------------------------------------------------------
...
DIR_ENT FAT_GetDirEntry ( u32 dirCluster, int entry, int origin)
{
	...
	int maxSectors;
	int lfnPos, aliasPos;
	u8 chkSumCalc; // modified by dragonminded, give a flag byte to tell if chksum is valid or not
	u16 lfnChkSum, chkSum;
	...
	do {
		...
		if ((dir.name[0] != FILE_FREE) && (dir.name[0] > 0x20) && ((dir.attrib & ATTRIB_VOL) != ATTRIB_VOL))
		{
			...
			if (lfnExists)
			{
				...
				{
					// NOTE: The operation is an unsigned char rotate right
					chkSumCalc = ((chkSumCalc & 1) ? 0x80 : 0) + (chkSumCalc >> 1) + (aliasPos < 8 ? dir.name[aliasPos] : dir.ext[aliasPos - 8]);
				}
				
				// set high byte
				chkSum = chkSumCalc | 0xFF00;
				
				if (chkSum != lfnChkSum) // edit by dragonminded, was picking up entries that didn't have lfn as a previous lfn entry
				{
					lfnExists = false;
					lfnName[0] = '\0';
				}
		...
		else if (dir.attrib == ATTRIB_LFN)
		{
			...
			else if (lfn.ordinal & LFN_END)	// Last part of LFN, make sure it isn't deleted (Thanks MoonLight)
			{
				...
				lfnChkSum = lfn.checkSum | 0xFF00;
			}
			if (lfnChkSum !=(lfn.checkSum | 0xFF00))
			{
				lfnExists = false;
			}
			if (lfnExists)
...
DIR_ENT FAT_DirEntFromPath (const char* path)
{
	...
	u32 tdirEntSector = (wrkDirCluster == FAT16_ROOT_DIR_CLUSTER ? filesysRootDir : FAT_ClustToSect(wrkDirCluster)) + wrkDirSector;
	int tdirEntOffset = wrkDirOffset;
	//iprintf("FAT_DirEntFromPath %d %d\n", tdirEntSector, tdirEntOffset);
	...
	while (!notFound && !found)
	{
		...
		tdirEntSector = (wrkDirCluster == FAT16_ROOT_DIR_CLUSTER ? filesysRootDir : FAT_ClustToSect(wrkDirCluster)) + wrkDirSector;
		tdirEntOffset = wrkDirOffset;
		//iprintf("Name %d %d ", tdirEntSector, tdirEntOffset);
		...
		while ( !found && !notFound)
		{
			...
			if (!found)
			{
				...
				tdirEntSector = (wrkDirCluster == FAT16_ROOT_DIR_CLUSTER ? filesysRootDir : FAT_ClustToSect(wrkDirCluster)) + wrkDirSector;
				tdirEntOffset = wrkDirOffset;
				//iprintf("Alias %d %d ", tdirEntSector, tdirEntOffset);
			...
			if (!found && !notFound)
			{
				...
				tdirEntSector = (wrkDirCluster == FAT16_ROOT_DIR_CLUSTER ? filesysRootDir : FAT_ClustToSect(wrkDirCluster)) + wrkDirSector;
				tdirEntOffset = wrkDirOffset;
				//iprintf("Loop %d %d ", tdirEntSector, tdirEntOffset);
...
bool FAT_AddDirEntry (const char* path, DIR_ENT newDirEntry)
{
	...
	filename[filePos] = 0; // null terminations help chishm (edit by dragonminded)
...
FAT_FILE* FAT_fopen(const char* path, const char* mode)
{
	...
	//iprintf("\n%d %d\n", file->dirEntSector, file->dirEntOffset);
...
// Ignore these completely unless for some reason you have to write to a file while listing a directory.
void FAT_preserveVars()
{
	fatLastCluster1 = fatLastCluster;
	curWorkDirCluster1 = curWorkDirCluster;
	wrkDirCluster1 = wrkDirCluster;
	wrkDirSector1 = wrkDirSector;
	wrkDirOffset1 = wrkDirOffset;
}

void FAT_restoreVars()
{
	fatLastCluster = fatLastCluster1;
	curWorkDirCluster = curWorkDirCluster1;
	wrkDirCluster = wrkDirCluster1;
	wrkDirSector = wrkDirSector1;
	wrkDirOffset = wrkDirOffset1;
}
