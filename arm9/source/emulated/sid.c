 ...
 *                     \/            \/     \/    \/            \/
 * $Id: sid.c 12498 2007-02-26 17:15:04Z jethead71 $
 *
...
static int internal_period, internal_order, internal_start, internal_end, internal_add, internal_repeat_times, internal_repeat_start;
...
void synth_render (s16 *buffer, unsigned long len)
{
...
#ifdef USE_FILTER
    filter.freq  = (4*sid.ffreqhi + (sid.ffreqlo&0x7)) * filtmul; // CHANGED used to be 16*
    ...
    for (v=0;v<3;v++) {
            ...
            outf+=((signed short)(outv-0x80)) * (osc[v].envval>>8); // CHANGED: was 4
#endif
        }
...
int sidGetCIA()
{
	return (int)(20000*(memory[0xdc04]|(memory[0xdc05]<<8))/0x4c00);
}

int sidGetMixFreq()
{
	return mixing_frequency; // even though this is just the value we passed before, I kept it just in case
}
