...
api68_t * api68_init(api68_init_t * init)
{
  ...
  if (init) {
    /* Set debug handler. */
    debugmsg68_set_handler(init->debug68);
...
int api68_seek(api68_t * api, int time_ms, int * is_seeking)
{
  ...
  if (time_ms == -1) {
    ...
    if (!api->mus) {
      ...
      int loop = 
	calc_loop_total(api->force_loop, api->loop_to, api->mus->loop);
    ...
    for (i=0, n=d->nb_six ; i<n; ++i) {
      ...
      int loop = 
	calc_loop_total(api->force_loop, api->loop_to, d->mus[i].loop);
	...
	if (ms > cur_ms) {
	  ...
	  api->seek_to = ms - start_ms;
	  api68_debug("SEEK-TO %d, cur:%d\n", api->seek_to, cur_ms);
	} else {
	  if (is_seeking) *is_seeking = 0;
	  api->seek_to = -1;
	  api68_debug("NO-SEEK-TO cur:%d\n", cur_ms);
	}
	return (int) cur_ms;
      }
    }
