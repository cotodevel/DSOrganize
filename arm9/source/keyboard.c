...
void toggleSelect()
{
	...
	if(select == 1)
	{
		...
		beginHighlight = getCursor();
		endHighlight = getCursor();
	}
	else
	{
		clearFakeHighlight();
...
void insertChar(char *tmpBuffer, int size, char c)
{
	if(isInsert())
	{
		...
		int z;
		for(z=size;z > getCursor();z--)
		{
			tmpBuffer[z] = tmpBuffer[z-1];		
		}
	}
	
	tmpBuffer[getCursor()] = c;
...
void genericAction(char *tmpBuffer, int size, char c)
{
	...
	else
	{
		...
		{
			// backspace
			if(getCursor() == 0) // handle backspacing nothing
			{
				return; 
			}
			
			int z;
			for(z=getCursor()-1;z<=size;z++)
			{
				tmpBuffer[z] = tmpBuffer[z+1];		
			}
		...
		{
			// del
			if(tmpBuffer[getCursor()] == 0) // handle deleting nothing
			{
				return; 
			}
