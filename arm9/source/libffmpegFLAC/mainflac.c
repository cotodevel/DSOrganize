...
bool flac_init(DRAGON_FILE *df, FLACContext* fc)
{
    ...
    if (DRAGON_fseek(df, 0, SEEK_SET) < 0)
        return false;

    if (DRAGON_fread(buf, 1, 4, df) < 4)
        return false;
    ...
    while (!endofmetadata) {
        ...
        if ((buf[0] & 0x7f) == 0)       /* 0 is the STREAMINFO block */
        {
            ...
            if (DRAGON_fread(buf, 1, blocklength, df) < 0)
                return false;
          
            fc->filesize = DRAGON_flength(df);
            ...
            DRAGON_fseek(df, blocklength, SEEK_CUR);
        } else {
            /* Skip to next metadata block */
            if (DRAGON_fseek(df, blocklength, SEEK_CUR) < 0)
                return false;
        }
    }
